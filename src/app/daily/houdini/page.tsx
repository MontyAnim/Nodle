...
import { useHoudiniStore } from "@/store/useGameStore";
...
export default function HoudiniDaily() {
  const t = useTranslations('Game');
  const tHub = useTranslations('Hub');
  const tKofi = useTranslations('Kofi');
  const resetDailyGame = useHoudiniStore((state) => state.resetDailyGame);
  const lastPlayedTimestamp = useHoudiniStore((state) => state.lastPlayedTimestamp);
  const attempts = useHoudiniStore((state) => state.attempts);
  const addAttempt = useHoudiniStore((state) => state.addAttempt);
  const hardMode = useHoudiniStore((state) => state.hardMode);
  const toggleHardMode = useHoudiniStore((state) => state.toggleHardMode);
  const colorblindMode = useHoudiniStore((state) => state.colorblindMode);
  const toggleColorblindMode = useHoudiniStore((state) => state.toggleColorblindMode);
  const debugDayOverride = useHoudiniStore((state) => state.debugDayOverride);
  const gameStatus = useHoudiniStore((state) => state.gameStatus);
  const setGameStatus = useHoudiniStore((state) => state.setGameStatus);
  const userId = useHoudiniStore((state) => state.userId);
  const dailyStartTime = useHoudiniStore((state) => state.dailyStartTime);
  const currentStreak = useHoudiniStore((state) => state.currentStreak);
  const maxStreak = useHoudiniStore((state) => state.maxStreak);
  const winDistribution = useHoudiniStore((state) => state.winDistribution);
  ...
  useEffect(() => {
    useHoudiniStore.setState(useHoudiniStore.getState());
  ...
  useEffect(() => {
    ...
    if (lastAttemptId === dailyNode.id) {
      if (gameStatus !== 'won') {
        console.log("[Leaderboard Houdini] Victory detected! userId:", userId, "dailyStartTime:", dailyStartTime);
        setGameStatus('won');
      ...
      <div className="w-full max-w-2xl flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="sr-only">{tHub('title')} - {t('houdini_daily')}</h1>
            <NodleLogo className="w-48 sm:w-56 drop-shadow-sm" nodeColor="text-amber-400" />
          </div>
          <p className="text-amber-400 text-sm sm:text-base font-medium">{t('houdini_daily')}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-2">
          <div className="flex items-center gap-3 bg-zinc-50 dark:bg-zinc-900 px-4 py-2 rounded-full border border-zinc-200 dark:border-zinc-800">
            ...
            <button 
              ...
              className={`w-10 h-5 rounded-full transition-colors relative flex items-center ${hardMode ? 'bg-amber-500' : 'bg-zinc-700'}`}
            >
