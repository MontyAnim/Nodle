...
export default function UnityDaily() {
  ...
  const currentStreak = useUnityStore((state) => state.currentStreak);
  const maxStreak = useUnityStore((state) => state.maxStreak);
  const winDistribution = useUnityStore((state) => state.winDistribution);
      ...
      <div className="w-full max-w-2xl flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="sr-only">{tHub('title')} - {t('unity_daily')}</h1>
            <NodleLogo className="w-48 sm:w-56 drop-shadow-sm" nodeColor="text-zinc-600 dark:text-zinc-400" />
          </div>
          <p className="text-zinc-600 dark:text-zinc-400 text-sm sm:text-base font-medium">{t('unity_daily')}</p>
        </div>
