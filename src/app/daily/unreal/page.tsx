...
export default function UnrealDaily() {
  ...
  const currentStreak = useUnrealStore((state) => state.currentStreak);
  const maxStreak = useUnrealStore((state) => state.maxStreak);
  const winDistribution = useUnrealStore((state) => state.winDistribution);
      ...
      <div className="w-full max-w-2xl flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="sr-only">{tHub('title')} - {t('unreal_daily')}</h1>
            <NodleLogo className="w-48 sm:w-56 drop-shadow-sm" nodeColor="text-indigo-400" />
          </div>
          <p className="text-indigo-400 text-sm sm:text-base font-medium">{t('unreal_daily')}</p>
        </div>
