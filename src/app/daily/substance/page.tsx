...
export default function SubstanceDaily() {
  ...
  const resetDailyGame = useSubstanceStore((state) => state.resetDailyGame);
  const lastPlayedTimestamp = useSubstanceStore((state) => state.lastPlayedTimestamp);
  const attempts = useSubstanceStore((state) => state.attempts);
  const addAttempt = useSubstanceStore((state) => state.addAttempt);
  const hardMode = useSubstanceStore((state) => state.hardMode);
  const toggleHardMode = useSubstanceStore((state) => state.toggleHardMode);
  const colorblindMode = useSubstanceStore((state) => state.colorblindMode);
  const toggleColorblindMode = useSubstanceStore((state) => state.toggleColorblindMode);
  const debugDayOverride = useSubstanceStore((state) => state.debugDayOverride);
  const gameStatus = useSubstanceStore((state) => state.gameStatus);
  const setGameStatus = useSubstanceStore((state) => state.setGameStatus);
  const userId = useSubstanceStore((state) => state.userId);
  const dailyStartTime = useSubstanceStore((state) => state.dailyStartTime);
  const currentStreak = useSubstanceStore((state) => state.currentStreak);
  const maxStreak = useSubstanceStore((state) => state.maxStreak);
  const winDistribution = useSubstanceStore((state) => state.winDistribution);

  const [allNodes, setAllNodes] = useState<NodeData[]>([]);
  const [substanceNodes, setsubstanceNodes] = useState<NodeData[]>([]);
  ...
  useEffect(() => {
    useSubstanceStore.setState(useSubstanceStore.getState());
    ...
    getNodes().then((nodes) => {
      setAllNodes(nodes);
      const filtered = nodes.filter(MODES_CONFIG.substance.filter);
      setsubstanceNodes(filtered);
      
      const target = getDailyTargetNode(nodes, currentDay, MODES_CONFIG.substance);
      setDailyNode(target);
      setIsReady(true);
    });
  ...
  useEffect(() => {
    ...
    if (lastAttemptId === dailyNode.id) {
      if (gameStatus !== 'won') {
        console.log("[Leaderboard Substance] Victory detected! userId:", userId, "dailyStartTime:", dailyStartTime);
        setGameStatus('won');
      ...
      <div className="w-full max-w-2xl flex flex-col gap-6">
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h1 className="sr-only">{tHub('title')} - {t('substance_daily')}</h1>
            <NodleLogo className="w-48 sm:w-56 drop-shadow-sm" nodeColor="text-red-400" />
          </div>
          <p className="text-red-400 text-sm sm:text-base font-medium">{t('substance_daily')}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 hover:border-red-500/60 text-red-400 rounded-full text-sm font-medium transition-all"
          >
