    ...
    <div
      className={`relative h-full flex flex-col p-6 rounded-2xl border transition-all duration-200 ${
        disabled
          ...
          : `bg-zinc-900/50 hover:bg-zinc-800/80 hover:-translate-y-1 hover:shadow-xl ${colorClass}`
      }`}
    >
      <div className="flex items-center justify-between mb-4">
        <div
          className={`p-3 rounded-xl ${
            disabled ? "bg-zinc-800 text-zinc-500" : "bg-zinc-800/50"
          }`}
        >
          <Icon className="w-6 h-6" />
        </div>
        {comingSoon && (
          <span className="px-3 py-1 text-[10px] uppercase tracking-wider font-bold rounded-full bg-zinc-800 text-zinc-400 border border-zinc-700">
            Pronto
          </span>
        )}
      </div>
